...
public class TiledMap {
	...
	/**
	 * Get a property given to a particular layer. Note that this method will not perform well and
	 ...
	 *            something else between lines (@see {@link #renderedLine(int, int, int)}
	 * 
	 *            TODO: [Isometric map] Render stuff between lines, concept of line differs from
	 *            ortho maps
	 */
	protected void renderIsometricMap(int x, int y, int sx, int sy, int width, int height,
			Layer layer, boolean lineByLine) {
		...
		while (!allProcessed) {
			...
			int min = 0;
			if (height > width) min = (startLineTileY < width - 1) ? startLineTileY : (width
					- currentTileX < height) ? width - currentTileX - 1 : width - 1;
			else min = (startLineTileY < height - 1) ? startLineTileY
			...
			for (int burner = 0; burner <= min; currentTileX++, currentTileY--, burner++) {
				for (int layerIdx = 0; layerIdx < drawLayers.size(); layerIdx++) {
					...
					currentLayer.render(currentLineX, initialLineY, currentTileX, currentTileY, 1,
							0, lineByLine, tileWidth, tileHeight);
				}
	...
	/**
	 * Looks for a property with the given name and returns it's value. If no property is found, def
	 * is returned.
	 * 
	 * @param groupID Index of a group
	 * @param objectID Index of an object
	 ...
	 * @return The value of the property with the given name or def, if there is no property with
	 *         that name.
	 */
